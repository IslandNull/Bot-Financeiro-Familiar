...
- Real deployment of the hardened Val Town proxy.
- Real Apps Script deployment of the narrow pilot expense path.
...
- Deploy the hardened Val Town proxy before the next pilot step; the currently verified real path proved `/help` delivery through Val Town, Apps Script, and Telegram before this final hardening patch.
- Push and redeploy the Apps Script runtime before sending a real expense message. The current patch specifically fixes real pilot `INVALID_DATE_EMPTY`, `PILOT_FLAGS_BLOCKED`, repeated-message `INVALID_MONEY`, and market false positives by defaulting omitted expense dates to today's Sao Paulo date, canonicalizing fixed pilot fields locally, hardening the prompt using V54 parser lessons, normalizing/extracting safe money values locally, and requiring market-like text alias confirmation. Keep `PILOT_FINANCIAL_MUTATION_ENABLED` unset or not `YES` until the new deployment is active and `/help` still works.
