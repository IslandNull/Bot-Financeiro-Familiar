...
- Version @114 deployed on 2026-05-19 with audit, pilot, clean rebuild, May launch hardening, `/resumo` exposure hardening, caixinha/cofrinho patrimonio updates, and Telegram decision-UX hardening:
...
- UX hardening is deployed: Telegram replies use the short `/resumo` pattern across help, launches, balances, agenda, review, safe questions, and validation failures; launch replies hide internal ids, explain card/invoice/cash impact in user language, and category forecast uses installment amount for monthly predictability while DRE keeps the full assumed purchase.
- Decision-UX @114 has read-only `/agenda`, `/revisar_mes`, deterministic "posso comprar ... em Nx?" simulations, and a short `/resumo`: current liquidity, confirmed current invoices, attention point, top forecast categories, next step, and drill-down commands.
- Latest validation after @114: `npm run check`, `npm run snapshot`, `npm run summary`, and `npm run selftest` passed on 2026-05-19.
