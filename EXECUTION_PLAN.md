...
- The Val Town proxy local contract reads Apps Script JSON `responseText` and returns a Telegram webhook `sendMessage` method response when possible. It forwards the webhook secret to Apps Script through both header and query parameter, uses explicit timeouts, validates HTTPS Apps Script targets, redacts operational diagnostics, disables Telegram web previews, protects against oversized Telegram response text, and avoids replying to failed auth gates. Local tests pass on 2026-04-30 with `npm test`; real hardening deployment remains unverified.
- User-reported operational status on 2026-05-02: the hardened Val Town proxy is deployed.
...
- User-reported operational status on 2026-05-02: `apps-script/appsscript.json` and `apps-script/Code.js` are deployed to Apps Script, and the unauthorized user/chat gate was validated successfully before parser/writer execution.
...
- Hardened Val Town proxy deployment is user-reported done on 2026-05-02; keep it as Telegram's webhook target for the next pilot step.
- Apps Script `appsscript.json` and `Code.js` deployment is user-reported done on 2026-05-02. The deployed patch specifically fixes real pilot `INVALID_DATE_EMPTY`, `PILOT_FLAGS_BLOCKED`, repeated-message `INVALID_MONEY`, and market false positives by defaulting omitted expense dates to today's Sao Paulo date, canonicalizing fixed pilot fields locally, hardening the prompt using V54 parser lessons, normalizing/extracting safe money values locally, and requiring market-like text alias confirmation. Keep `PILOT_FINANCIAL_MUTATION_ENABLED` scoped to the reviewed pilot gates.
...
2. Unauthorized chat test. Local handler/gate status: VERIFIED locally on 2026-04-29 with `npm test`; Web App URL test status: user-reported validated on 2026-05-02 before parser/writer execution.
