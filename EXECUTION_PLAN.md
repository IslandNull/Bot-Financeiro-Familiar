...
- Version @111 deployed on 2026-05-18 with audit, pilot, clean rebuild, May launch hardening, `/resumo` exposure hardening, caixinha/cofrinho patrimonio updates, and Telegram decision-UX hardening:
...
- Decision-UX @111 adds read-only `/agenda`, `/revisar_mes`, deterministic "posso comprar ... em Nx?" simulations, and a short `/resumo`: current liquidity, confirmed current invoices, attention point, top forecast categories, next step, and drill-down commands.
- Latest validation after @111: `npm run check`, `npm run snapshot`, `npm run summary`, and `npm run selftest` passed on 2026-05-18.
