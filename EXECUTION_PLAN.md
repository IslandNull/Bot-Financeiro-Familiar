...
- Version @115 deployed on 2026-05-19 with audit, pilot, clean rebuild, May launch hardening, `/resumo` exposure hardening, caixinha/cofrinho patrimonio updates, and Telegram decision-UX hardening:
...
- Decision-UX @115 has read-only `/agenda`, `/revisar_mes`, deterministic "posso comprar ... em Nx?" simulations, category-specific spending explanations, and a short `/resumo`: current liquidity, confirmed current invoices, attention point, top forecast categories, next step, and drill-down commands.
- Latest validation after @115: `npm run check`, `npm run snapshot`, `npm run summary`, and `npm run selftest` passed on 2026-05-19.
